...
class Cache:
    ...
    def replay(self, func):
        ...
        input_args_list = self._redis.lrange("{}:inputs"
                                             .format(func_name), 0, -1)
        output_list = self._redis.lrange("{}:outputs"
                                         .format(func_name), 0, -1)

        print("Function {} was called {} times:"
              .format(func_name, len(input_args_list)))
        for input_args, output in zip(input_args_list, output_list):
            print("{}{} -> {}".format(func_name, input_args.decode(),
                                      output.decode()))
